...
public class GHNApiUtil {
    ...
    public static double totalFee(String address) {
        ...
        try {
            // Lấy mã tỉnh
            int provinceId = GHNApiUtil.findProvinceIdByName(provinceName);
            if (provinceId == -1) {
                System.out.println("Không tìm thấy tỉnh: " + provinceName);
                return -1;
            }

            // Lấy mã huyện
            int districtId = GHNApiUtil.findDistrictIdByName(provinceId, districtName);
            if (districtId == -1) {
                System.out.println("Không tìm thấy huyện: " + districtName);
                return -1;
            }

            // Lấy mã xã
            int wardId = GHNApiUtil.findWardIdByName(districtId, wardName);
            if (wardId == -1) {
                System.out.println("Không tìm thấy xã: " + wardName);
                return -1;
            }
